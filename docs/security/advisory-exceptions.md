...
| RUSTSEC-2023-0071 | `rsa@0.9`                    | accept   | RSA Marvin Attack (timing sidechannel) transitive via `tools/issues-sync/` → `octocrab 0.51` → `jsonwebtoken 10.4` → `rsa 0.9`. Attack profile (network-observable timing on key operations) does not apply — `tools/issues-sync` is an outbound HTTPS client running in CI against GitHub; no key material is exposed to remote timing observation. No upstream patch (tracked at github.com/RustCrypto/RSA/issues/626). Re-evaluate on `rsa 1.x` release or octocrab dropping `jsonwebtoken`. Story 1.16 / LD-55 surface.                                                                                                                                                | 2026-05-29     | 2026-08-27  |
| RUSTSEC-2026-0194 | `quick-xml@0.39.4`           | accept   | Quadratic runtime on duplicate-attribute check in untrusted XML. The runtime path (`tauri` → `plist`) was fixed by the Story 3.1 lockfile bump `plist 1.10` → `quick-xml 0.41`. The residual 0.39.4 is pinned solely by `wayland-scanner 0.31` — a proc-macro that parses vendored Wayland protocol XML at build time on Linux builders; no untrusted input ever reaches it. Re-evaluate when wayland-rs bumps quick-xml.                                                              | 2026-07-14     | 2026-10-12  |
| RUSTSEC-2026-0195 | `quick-xml@0.39.4`           | accept   | Unbounded namespace-declaration allocation (memory-exhaustion DoS) in `NsReader` on untrusted XML. Same root cause and same containment as RUSTSEC-2026-0194 (wayland-scanner build-time parse of trusted vendored XML only).                                                                                                                                                                                                                                                          | 2026-07-14     | 2026-10-12  |
| RUSTSEC-2026-0235 | `rkyv@0.7.46`                | accept   | Insufficient archive validation → out-of-bounds reads in archives containing `Rc`/`Arc`. `rkyv` is an OPTIONAL dependency of `rust_decimal` with its feature OFF, so it is not compiled into any build — a lockfile-only phantom (`cargo tree -i rkyv` prints nothing). Transitive lock root: `byte-unit` → `tauri-plugin-log` → `orgsidian-shell-app`. No rkyv archive is ever deserialized (the vulnerability requires decoding an untrusted rkyv archive). Surfaced during Story 3.4 (unrelated to that change). Re-evaluate when `tauri-plugin-log`/`byte-unit` drop the `rust_decimal` edge or `rust_decimal` bumps `rkyv` to >=0.8.17.                                                                    | 2026-08-04     | 2026-11-02  |
...
| `quick-xml@0.39.4` (wayland-scanner 0.31 proc-macro, Linux build-time; main tree on 0.41 via plist 1.10)                                | accept   | 2026-07-14     | 2026-10-12  |
| `rand@0.8.6`, `rand_chacha@0.3.1`, `rand_core@0.6.4` (fail 0.5, opt-in `failpoints` only; atomic-write-file uses rand 0.9 — Story 3.1)  | accept   | 2026-07-14     | 2026-10-12  |
