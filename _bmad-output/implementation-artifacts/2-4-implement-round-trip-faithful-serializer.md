...
- [x] **T1** — Re-verify the tiling probe data from Dev Notes §1 with a throwaway scratch test (the 2.2/2.3 `tests/zz_scratch.rs` pattern, deleted before commit): preamble/section tiling on blank-line, CRLF, no-trailing-newline, empty, ERROR-region inputs. Confirms nothing changed under you; costs minutes; the probe code is reproduced in Dev Notes §1. (AC2)
- [x] **T2** — Add `raw: String` to `Headline` in `src/semantic/headline.rs`: populate in `build_section` from the existing own-region boundaries (links-scan region, lines 254-259); document field + invariant. Implement gap-absorption in `src/semantic/mod.rs` `analyze()` (position cursor over top-level pieces) and in the child walk, so `None`-section and ERROR-node bytes are never dropped. (AC2)
- [x] **T3** — Create `src/serializer.rs`: `//! Implements FR-2` header, `serialize(headlines: &[Headline]) -> String` (recursive raw concat), `serialize_document(&Document) -> String` (preamble + headlines). Wire `lib.rs` (`mod serializer;` + re-exports + header prose present-tense). (AC1, AC5)
- [x] **T4** — Build the interim corpus `tests/fixtures/round_trip/*.org` (~10-20 files: 15 LD-44 constructs + edge bucket per AC3); add the `.gitattributes` `-text` rule with scoping comment. (AC3)
- [x] **T5** — Write `tests/round_trip.rs`: `round_trip_subset` corpus test (directory-driven, divergence diagnostics) + inline-literal cases for CRLF / mixed endings / missing trailing newline / empty input. (AC3)
- [x] **T6** — Add `proptest = { workspace = true }` to parser `[dev-dependencies]` (Story-2.4 comment); write the epic idempotence property (generated org content → `analyze` → `serialize` → `analyze` → `serialize`, second == first) + the arbitrary-input identity property (`serialize_document(analyze(s)) == s` for any `String`), 256 cases each, settings_round_trip.rs house style. (AC4)
- [x] **T7** — Docs hygiene: `///` on all new pub items; module-doc design contract incl. zero-normalization posture + variances; `cargo doc -p orgsidian-parser --no-deps` clean. (AC5)
...
Claude Fable 5 (claude-fable-5[1m]) via Claude Code / BMad dev-story workflow.

### Debug Log References

- T1 tiling probe (throwaway `tests/zz_scratch.rs`, deleted before commit) re-verified Dev Notes §1 at the pinned SHA: all well-formed cases (two-section, preamble+blanks, no-trailing-newline, trailing blanks, CRLF, preamble-only, nested, blank-between-sections, empty, leading blanks, lone-stars, stars-no-space) tile 0..len exactly. One pathological case confirmed the AC2 hazard: `"\0\u{1}* X\n\u{7f}"` → root-level `ERROR` node 0..3 + `body` 4..7 = gap 0..4 without absorption.
- RED phase: `tests/round_trip.rs` written against a stub `serialize`/`serialize_document` returning `String::new()` — 12/14 tests failed as expected (the 2 greens were empty-input and the fixture tripwire, which don't exercise the serializer). GREEN after the real implementation: 14/14 in 0.32s.
- proptest-regressions file generated during the RED phase (seeds shrunk to `"* A\n"`, single chars — stub artifacts, not real bugs) deleted per Dev Notes §5 house precedent; all properties re-verified green from fresh randomness.

### Completion Notes List

- **AC1** — `src/serializer.rs` (NEW, top-level): `pub fn serialize(headlines: &[Headline]) -> String` (epic-verbatim signature) + `pub fn serialize_document(document: &Document) -> String` (preamble + headlines), both re-exported from `lib.rs` mirroring the `analyze` pattern. Pure raw-region passthrough: recursive `raw` + children concat; infallible, no I/O, no semantic-field re-emission. 4 unit tests for region math/emission order in the module.
- **AC2** — `Headline.raw: String` populated in `build_section` from the SAME own-region boundaries as the links scan (single source of truth, no drift). Gap-absorption: position cursor in `analyze()` (top level) and in `build_section`'s child walk; uncovered bytes are prepended to the next retained region's `raw`, appended after the last emission (via `absorb_trailing`, which recurses to the last descendant to preserve byte order), absorbed into the preamble, or — when nothing is retained at all — synthesized into a preamble. Field documented with own-region definition + tiling invariant.
- **AC3** — `tests/round_trip.rs` with corpus test **`round_trip_subset`** (directory-driven over `tests/fixtures/round_trip/`, ≥10-file floor assert, divergence diagnostics: filename + first divergent byte offset + ±20-byte context windows). Interim corpus: 18 handcrafted fixtures covering all 15 LD-44 constructs (samples adapted from `tests/semantic.rs`, no invented syntax) + edge bucket (Unicode/RTL, over-indented drawer, trailing whitespace in headlines, no-trailing-newline file, preamble-only, deep nesting 1-6, kitchen sink with leading/trailing blanks). CRLF / mixed-EOL / missing-trailing-newline / empty / whitespace-only / ERROR-region cases live as inline `&str` literals. `.gitattributes` gained the narrow-scoped `-text` rule (with rationale comment mirroring the perf-baselines entry) + a `corpus_retains_byte_sensitive_fixtures` tripwire test that fires if EOL protection is ever bypassed.
- **AC4** — Both properties implemented in `tests/round_trip.rs`, 256 cases each, settings_round_trip.rs house style (named strategies, `proptest!` + explicit `ProptestConfig`): (1) epic idempotence — generated org text (levels 1-6, optional default-set TODO keywords, titles, tags, planning lines with chrono-valid dates + repeaters, property drawers, bodies; nesting from random level sequences) → `analyze` → `serialize` → `analyze` → `serialize`, second == first (plus a first==input identity assert, free via tiling); (2) arbitrary-input identity `serialize_document(&analyze(s)?) == s` over `any::<String>()` plus a `\PC{0,200}` Unicode-heavy variant. No grammar-level blocker surfaced; no counterexample from the scanner.c indent bug (byte coverage survives ERROR trees, as predicted). `proptest = { workspace = true }` dev-dep edge with Story-2.4 comment; Cargo.lock delta = exactly 1 line (the edge), zero new crates.
- **AC5** — `serializer.rs` first doc line is `//! Implements FR-2` (single FR-2 trace owner); module docs state raw-passthrough contract, tiling invariant, why field-driven rendering is forbidden, and the zero-normalization posture (FR-2 "modulo trailing-newline" allowance NOT exercised; no Settings surface needed — module doc is the documentation of record). `lib.rs` header prose now present-tense; `Implements FR-1` line intact. All new pub items have `///` docs; `cargo doc -p orgsidian-parser --no-deps` clean (no warnings).
- **AC6** — All gates green: `cargo build -p orgsidian-parser --locked` ok; `cargo test -p orgsidian-parser --locked` = **77 passed / 0 failed** (baseline 59 unchanged: 1 anchor + 4 grammar + 24 semantic + 30 unit; NEW: +4 serializer unit tests → 34 unit, +14 round_trip = 1 corpus `round_trip_subset` + 1 fixture tripwire + 9 inline edge cases + 3 proptest properties), suite <1s warm (round_trip 0.33s). `cargo test --workspace --locked` = **122 passed / 0 failed / 11 ignored** (baseline 104/11, +18, no regressions). clippy workspace all-targets clean; `cargo fmt --check` clean; no `unwrap()`/`expect()`/`println!` in library code. `cargo deny check licenses bans advisories` = ok/ok/ok; `cargo audit` at the 18-allowed-warnings baseline. Sentinel check via `git status`: `tests/anchor.rs`, `tests/grammar.rs`, `tests/semantic.rs`, `tests/fixtures/anchor.org`, `build.rs`, `grammar/` all untouched.
- **AC7** — deferred-work.md: `Timestamp::end_time` item annotated (field split NOT forced by 2.4 — raw passthrough never consumes `Timestamp` fields; decision stays with the Epic 4 picker consumer); story-2.4 stanza pre-seeded (mutation-aware serialization → Epic 4; corpus expansion → Story 2.5; gap-absorption shape-lossiness on pathological trees → diagnostics consumer, if ever).
- **Memory posture note (Dev Notes §3):** `raw` roughly doubles owned-string cost per headline — continuation of Story 2.3's owned-data decision (LD-42 bounded-by-file-size), not a new decision. `Document` does NOT retain the full source (no placebo serializer).
- **Variances recorded (Dev Notes §7, no spec edits):** (1) `serialize_document` added as document-level entry point (epic signature kept verbatim); (2) interim 18-file corpus now, full subset via 2.5/2.6 into the same directory-driven `round_trip_subset`; (3) zero-normalization posture documented in module docs (no Settings surface exists/needed); (4) proptest headlines generated via rendered-org-text + `analyze()` (the only honest constructor of a valid `raw`).
- **Process deviations (orchestration-level, recorded):** T10's "open PR" and the GitHub-issue label flip (`status:in-progress`/`status:review` on issue #20) were NOT executed — the run's task rules forbid creating/modifying GitHub PRs/issues and pushing; issue #20 was verified to exist (read-only). Commit made locally on `story/2.4-round-trip-serializer` with the prescribed conventional message; PR + label sync are left to the pipeline's next step.
- **Preamble doc touch-up:** `Preamble.text` doc now notes the (pathological-input-only) gap-absorption extension — accuracy follow-through of AC2, zero behavior change for well-formed input.

### File List

- `crates/orgsidian-parser/src/serializer.rs` — NEW: FR-2 serializer (`serialize`, `serialize_document`, recursive `emit`, 4 unit tests).
- `crates/orgsidian-parser/src/semantic/headline.rs` — UPDATE: `Headline.raw` field + docs; `raw` population from links-scan boundaries; gap-absorbing child walk; `absorb_trailing` helper.
- `crates/orgsidian-parser/src/semantic/mod.rs` — UPDATE: top-level gap-absorption cursor in `analyze()` (preamble backward extension, headline prefix absorption, trailing absorption, synthesized preamble for all-ERROR input); `Preamble.text` doc note.
- `crates/orgsidian-parser/src/lib.rs` — UPDATE: `mod serializer;` + `pub use serializer::{serialize, serialize_document};`; crate-header prose present-tense.
- `crates/orgsidian-parser/Cargo.toml` — UPDATE: `[dev-dependencies] proptest = { workspace = true }` (Story-2.4 comment).
- `crates/orgsidian-parser/tests/round_trip.rs` — NEW: `round_trip_subset` corpus test + fixture tripwire + 9 inline edge cases + 3 proptest properties (256 cases each).
- `crates/orgsidian-parser/tests/fixtures/round_trip/*.org` — NEW: 18 interim corpus fixtures (01_todo_levels … 18_no_trailing_newline).
- `.gitattributes` — UPDATE: scoped `-text` rule for the round-trip fixture corpus.
- `Cargo.lock` — UPDATE: proptest dev-dep edge on orgsidian-parser (1 line, zero new crates).
- `_bmad-output/implementation-artifacts/deferred-work.md` — UPDATE: `Timestamp::end_time` 2.4 decision annotation + story-2.4 stanza.
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — UPDATE: story 2-4 status transitions.
- `_bmad-output/implementation-artifacts/2-4-implement-round-trip-faithful-serializer.md` — UPDATE: this story file (permitted sections only).
...
- 2026-06-10 — Story 2.4 implemented (raw-region retention + gap-absorption, FR-2 serializer, `round_trip_subset` corpus harness + 18-fixture interim corpus, idempotence + arbitrary-input identity proptest properties). Parser suite 59 → 77 tests, workspace 104 → 122 passed, all gates green, sentinels untouched. Status: review.
