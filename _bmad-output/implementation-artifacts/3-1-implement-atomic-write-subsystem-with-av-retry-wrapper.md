...
- [x] [Review][Decision] Orphan matcher can delete legitimate user files whose names coincidentally fit the temp pattern — the suffix rule "exactly 6 ASCII alphanumerics" matches plain English words: a user file `.notes.org.backup` ("backup" = 6 alnum) or `.todo.org.old123` is deleted once ≥60s old. No target-existence guard (`notes.org` present in the same dir) is checked, and no false-positive fixture exists in the test suite. Inherent residual risk of the story-approved pattern+mtime deviation, but data loss in a notes app is the worst failure class. Decide: add a target-existence guard now (orphans whose target never existed would then survive — acceptable residue) / accept + document + defer to Story 3.6 wiring / other.
- [x] [Review][Decision] Cleanup scan error policy: any single per-entry failure aborts the whole scan and discards the partial `CleanupReport` — one permission-denied subdirectory, a dir removed mid-scan, or a TOCTOU `NotFound` on `remove_file` (two instances opening the same vault) returns `Err`, losing the record of files *already deleted*. Story 3.6 wires this into Vault open, where a hard error over one bad entry is hostile. Decide: best-effort semantics (skip-and-record errors in the report) / fail-fast kept but `NotFound` treated as skip / defer policy to Story 3.6.
- [x] [Review][Patch] `is_transient_lock` doc/code mismatch on raw 32/33 gating: doc comment (and AC3 letter) say raw codes count only for uncategorized kinds; code checks `raw_os_error()` on any kind (EPIPE on Unix would retry). Behavior is deliberate and defensible (kind-gating on `Uncategorized` is unstable/fragile) — align the doc comment and record the AC3 letter-variance in Completion Notes; no behavior change. [crates/orgsidian-vault/src/atomic.rs (is_transient_lock)]
- [x] [Review][Patch] `disk_full_atomic_write` asserts only `is_err()`; the LD-41 row it proves says "fails immediately, no retry" — assert the error is `VaultError::Io { .. }` (not `RetriesExhausted`) so a future misclassification of ENOSPC as transient cannot pass green after 300ms of retries. [tests/failure_modes.rs:43]
- [x] [Review][Patch] Fail-point payload is decorative: `fail::cfg(..., "return(ENOSPC)")` payload string is ignored by the closure `|_| Err(StorageFull)` — a future `return(EACCES)` config would silently still inject ENOSPC. Align: drop the payload or comment that it is documentation-only. [tests/failure_modes.rs:34 + crates/orgsidian-vault/src/atomic.rs:79]
- [x] [Review][Patch] `clean_orphans_in_dir` recurses unbounded — a pathologically deep tree (sync-tool nesting loops) overflows the stack and aborts the process instead of erroring. Convert to iterative worklist traversal. [crates/orgsidian-vault/src/atomic.rs:194-216]
- [x] [Review][Patch] Symlink-to-directory entries are silently skipped by the orphan scan (`file_type()` doesn't follow symlinks) — orphans in symlinked org folders persist forever and the LD-41 guarantee silently doesn't hold there. Non-follow is defensible (cycle safety); document the choice + add a test asserting it. [crates/orgsidian-vault/src/atomic.rs:213-218]
- [x] [Review][Patch] `CleanupReport.removed` doc claims absolute paths but yields relative paths for a relative `vault_root` — fix the doc contract before Story 3.6 consumes it. [crates/orgsidian-vault/src/atomic.rs:164-169]
- [x] [Review][Patch] `VaultError` lacks `#[non_exhaustive]` — Story 3.2+ grows this crate; every new variant is a breaking change for downstream exhaustive matches. Cheap now. [crates/orgsidian-vault/src/error.rs]
- [x] [Review][Patch] Deferred-work hygiene: three Story-1.11 items owned by "Story 3.1 / first LD-41 implementation story" were left unannotated — the `fail::cfg` exemplar item is de-facto resolved by this PR (mark CLOSED); the anti-placebo gap and decorative `required-features` items were not addressed (re-own explicitly). Also: add a line for non-`.org` temp residue (settings `.toml` temps are never collected — tests assert survival, no entry covers it) and append the per-retry multiplication nuance (an AV-exhausted save can strand up to 3 temps; harness covers the write-body path only) to the commit-residue stanza. [_bmad-output/implementation-artifacts/deferred-work.md:68-72,206-209]
- [x] [Review][Defer] Case-sensitive `.org` match in orphan matcher misses uppercase-extension targets (`NOTES.ORG` temps never cleaned) [crates/orgsidian-vault/src/atomic.rs:254] — deferred, case-handling policy undecided until the index stories.
- [x] [Review][Defer] Coverage matrix drops implemented rows entirely instead of marking them covered, so the matrix only records gaps [docs/failure-modes/coverage-matrix.md + scripts/gen-failure-modes-matrix.mjs] — deferred, pre-existing generator design.
...
- **Variance disclosure (Dev Note 8):** (1) AC5 orphan pattern deviation as above; (2) `atomic_write` return-type change supersedes the Story-1.9 lib.rs doc claim — doc comment rewritten, epic-mandated; (3) **AC3 letter variance (2026-07-14 code review):** the raw 32/33 check in `is_transient_lock` is deliberately NOT gated on an uncategorized `ErrorKind` — `ErrorKind::Uncategorized` is unstable and its mapping can shift across Rust releases, so kind-gating would risk silently breaking the Windows retry path; on Unix those codes (EPIPE/EDOM) never arise from an atomic file write. Behavior kept, doc comment made explicit.
- **Code-review hardening (2026-07-14, post-review patches):** orphan cleanup gained a **target-existence guard** (a pattern-shaped name with no `{stem}.org` sibling is never deleted — closes the `.archive.org.backup` false-positive class; residue of a crashed first save to a brand-new target is the accepted trade-off) and **best-effort scan semantics** (`CleanupReport.errors: Vec<(PathBuf, io::Error)>` records per-entry failures — unreadable subdir, undeletable orphan — instead of aborting and discarding the partial report; `NotFound` on remove is a benign race, skipped; only an unreadable `vault_root` is a hard error). Traversal is now iterative (no stack overflow on pathological nesting); symlinks documented+tested as never followed; `VaultError` is `#[non_exhaustive]`; `disk_full_atomic_write` asserts the error is `VaultError::Io` (non-retried) and the fail-point cfg uses a bare `return` action (payload was decorative).
...
- 2026-07-14 — Adversarial code review (PR #146): 2 decision + 8 patch findings resolved and applied (orphan-cleanup target-existence guard + best-effort scan semantics with `CleanupReport.errors`, iterative traversal, symlink non-follow documented+tested, `#[non_exhaustive]` VaultError, harness assert/fail-point tightening, AC3 classifier variance disclosed, deferred-work hygiene); 2 deferred (case-sensitive `.org` match; coverage-matrix drops-implemented-rows design); 7 dismissed. All gates re-run green. Status → done.
