...
_From `bmad-code-review` pass on 2026-06-01 (Blind Hunter + Edge Case Hunter + Acceptance Auditor against PR #135). 14 dismissed as noise._

- [x] [Review][Decision] **"Nearest preceding bg" pairing structurally weak** — AC1's "nearest preceding `--org-bg-*`" rule means the impl is spec-compliant, but in current `tokens.css` all 3 bg tokens are declared first so every `:root` fg is silently paired against `--org-bg-elevated` (`#ffffff`). The gate cannot detect a regression where `--org-fg-muted` would fail on `--org-bg-surface` (`#f7f7f7`). Options: (a) keep spec rule, document the foot-gun + ordering invariant in `tokens.css` header; (b) require explicit `@pair-bg: <token>` annotation alongside `@pair-role`; (c) pair every fg against every preceding bg in the block. [shell-ui/src/themes/contrast.test.ts + tokens.css]
- [x] [Review][Decision] **Un-fixme discipline — surface-discriminator missing from scaffolds** — when a downstream story removes `.fixme` on (say) `today-dashboard.spec.ts`, `page.goto('/today')` against a 404/placeholder will yield zero `serious|critical` axe violations → gate passes green against a non-existent surface. Options: (a) add a TODO discriminator placeholder (e.g. `await expect(page.getByRole('heading')).toBeVisible()`) in each of the 6 scaffolds before the axe scan; (b) leave scaffolds as-is and trust downstream-story devs to add discriminators; (c) add a separate CI grep guard that asserts the 6 spec files exist and count `@a11y` tags. [shell-ui/e2e/a11y/*.spec.ts]
- [x] [Review][Patch] Add floor assertion on `block.pairs.length >= 1` in contrast test so a parser regression cannot yield a vacuous green gate [shell-ui/src/themes/contrast.test.ts]
- [x] [Review][Patch] `findUnpaired` regex misses `--org-border-*` tokens; extend it (or derive `unpaired` from `parseTokens` output) so ui-chrome borders without `@pair-role` are flagged [shell-ui/src/themes/contrast.test.ts]
- [x] [Review][Patch] `findUnpaired` `pendingRole` reset diverges from `parseTokens` — only resets when an fg-named token consumes the role, so an intervening `--org-border-*` declaration leaks the role to the next fg silently [shell-ui/src/themes/contrast.test.ts]
- [x] [Review][Patch] Playwright browser-cache key hashes `shell-ui/package.json` (caret-pinned) but not `pnpm-lock.yaml`; a lockfile-only Playwright bump leaves a stale `~/.cache/ms-playwright/chromium-XXXX` → "Executable doesn't exist" [.github/workflows/pr.yml]
- [x] [Review][Patch] Dev Agent Record's "dep pin policy" variance lists `vitest@^4.1.7` + `jsdom@^29.1.1` but omits the parallel `@playwright/test@^1.60.0` (spec said `^1.50.0`) and `@axe-core/playwright@^4.11.3` (spec said `^4.10.0`); update for honesty [story file metadata]
- [x] [Review][Patch] `quick-capture.spec.ts` fabricates `await page.goto('/quick-capture')` where AC4's mapping table left the route deliberately TODO (Tauri-window); restore the TODO literal or document the placeholder [shell-ui/e2e/a11y/quick-capture.spec.ts]
- [x] [Review][Defer] Selector regex `[^}]*` cannot handle nested braces — Story 6.7 fragility, current `tokens.css` doesn't trigger [shell-ui/src/themes/contrast.test.ts] — deferred, future scope
- [x] [Review][Defer] Dark theme `.dark` block validated by gate #1 but unreachable at runtime (no theme toggle); gates #2/#3 give zero dark-mode axe coverage until toggle ships [shell-ui/src/themes/tokens.css] — deferred, Story 6.7 / theme toggle scope
- [x] [Review][Defer] Workflow Step 14 (`Build shell-ui`) → Step 16 (`a11y`) ordering is load-bearing (dev-server boot depends on generated files from Step 14) but undocumented in `pr.yml` [.github/workflows/pr.yml] — deferred, add inline comment in a future pr.yml refactor
- [x] [Review][Defer] `sprint-status.yaml` change in this PR also flips Stories 1-10 and 1-14 from `review` → `done`, orthogonal to Story 1.17 scope [_bmad-output/implementation-artifacts/sprint-status.yaml] — deferred, sprint-tracking hygiene, separate concern
- [x] [Review][Defer] No `upload-artifacts` step for Playwright traces/HTML report on CI failure [.github/workflows/pr.yml] — deferred, quality-of-life when downstream stories awaken scenarios
...
- **AC variance recorded (dep pin policy)**: four dep pins bumped from spec literals to current latest stable per `[[feedback_version_policy]]` and the AC2/AC3 "verify with `pnpm view` before pinning" directive — `vitest@^4.1.7` (spec `^3.0.0`), `jsdom@^29.1.1` (spec `^25.0.0`), `@playwright/test@^1.60.0` (spec `^1.50.0`), `@axe-core/playwright@^4.11.3` (spec `^4.10.0`). All four are still MIT-licensed (LD-37 allowlist), and `pnpm install --frozen-lockfile` is reproducible against the committed `pnpm-lock.yaml`.
...
- **Post-review patches (2026-06-01, bmad-code-review)**: 8 review findings applied:
  - **AC1/AC2 convention upgrade — explicit `@pair-bg: <token>`**: replaced the "nearest preceding `--org-bg-*`" heuristic with an explicit `@pair-bg: --org-bg-<name>` annotation that every non-bg `--org-*` token must carry alongside `@pair-role`. Closes the silent-cross-bg-regression hole flagged by the Blind Hunter. `tokens.css` updated to annotate every fg/border with its semantic bg (`--org-fg-muted` and `--org-fg-subtle` now explicitly paired against `--org-bg-surface`, exercising the actual sidebar/panel contrast); `contrast.test.ts` parser rewritten to consume both annotations and to surface `missing-role` / `missing-bg` / `unknown-bg` errors on any unpaired token. Tests now: 16/16 pass (10 pair tests = 5 pairs × 2 themes + 3 structural + 3 sanity).
  - **Vacuous-gate floor**: added `MIN_PAIRS_PER_BLOCK = 5` assertion in `contrast.test.ts` so a parser regression cannot yield zero-pair-but-green output.
  - **`findUnpaired` unification**: removed the separate `findUnpaired` helper (regex was too narrow — missed `--org-border-*` tokens — and its `pendingRole` reset diverged from `parseTokens`). `unpaired` is now a field on each `SelectorBlock` produced by the single `parseTokens` pass, eliminating two-passes drift.
  - **CI cache key broadened**: `.github/workflows/pr.yml` Playwright cache key now hashes both `shell-ui/package.json` AND `pnpm-lock.yaml` so a lockfile-only transitive bump busts the cache instead of restoring a stale browser binary.
  - **@a11y inventory grep guard**: added a pre-`pnpm a11y` step that asserts exactly 6 spec files in `shell-ui/e2e/a11y/` carry the `@a11y` tag — protects the LD-58 inventory from silent file-deletion regressions.
  - **`quick-capture.spec.ts` placeholder honesty**: replaced the fabricated `await page.goto('/quick-capture')` with a TODO-marked, commented-out goto plus a surface-discriminator example that Story 8.1 must add before un-fixme-ing. Restores AC4's mapping-table TBD posture.
  - **Dep pin variance disclosure**: dep-pin AC variance above expanded to cover all four bumped pins (vitest + jsdom + `@playwright/test` + `@axe-core/playwright`), closing the Acceptance Auditor finding.
  - **5 findings deferred** to follow-ups: nested-brace selector regex (Story 6.7), dark-theme axe coverage (Story 6.7 + theme toggle), workflow Step-14 → Step-16 ordering comment, sprint-status hitchhiking, Playwright upload-artifacts. Tracked in `_bmad-output/implementation-artifacts/deferred-work.md`.
...
- `.github/workflows/pr.yml` — appended Step 16 (Story 1.17, LD-58): @a11y inventory guard + cache-playwright-browsers (key includes pnpm-lock.yaml) + install-chromium + `pnpm a11y` gate.
...
- 2026-06-01: Post-review patches applied (bmad-code-review pass; 2 decisions + 6 patches resolved, 5 findings deferred, 14 dismissed as noise). Hardenings: (a) `tokens.css` + `contrast.test.ts` upgraded from "nearest preceding bg" heuristic to explicit `@pair-bg: <token>` annotation — closes silent cross-bg regression hole; `--org-fg-muted` / `--org-fg-subtle` now actually paired against `--org-bg-surface`. Contrast test goes from 15 to 16 cases (10 pair × 2 themes + 3 structural + 3 sanity), all green. (b) `MIN_PAIRS_PER_BLOCK = 5` floor assertion prevents future vacuous-gate regression. (c) `findUnpaired` helper removed, unified into the single `parseTokens` pass (eliminates two-passes drift; now flags `--org-border-*` tokens that were previously regex-missed). (d) Playwright cache key broadened to `hashFiles('shell-ui/package.json', 'pnpm-lock.yaml')`. (e) New CI step asserts the 6 `@a11y` spec files are present (LD-58 inventory drift guard). (f) `quick-capture.spec.ts` fabricated `/quick-capture` route replaced with a TODO-marked commented-out goto + surface-discriminator example for Story 8.1. (g) Dep-pin AC variance expanded to disclose all 4 bumped pins.
