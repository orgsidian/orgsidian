...
| `full-nightly.json` | Pointer manifest over the full nightly corpus (~one entry per harvested `test-org-element.el` assertion; paths into `tests/fixtures/vault-corpus/`). | Story 2.7's nightly full-corpus + L2 oracle gates; `verify`. |
